...
models           <- new.env()
modesl$roughness <- readRDS(paste0(gbm_dir, "roughness.rds"))$finalModel
modesl$freq      <- readRDS(paste0(gbm_dir, "freq.rds"))$finalModel
modesl$freq_y    <- readRDS(paste0(gbm_dir, "freq-y.rds"))$finalModel

#' @keywords internal
.parameterize <- function(pathlength, arbolatesu, lengthkm, areasqkm, slope) {
    data.frame(
...
.predict <- function(df, model) {
    exp(gbm::predict.gbm(object = model, newdata = df))
}
...
function(pathlength, arbolatesu, lengthkm, areasqkm, slope) {
    .predict(
        .parameterize(pathlength, arbolatesu, lengthkm, areasqkm, slope),
        models$roughness
    )
}
